...
/************************************************************************/
/****************************************************************************
* 名    称：void des(uint8_t *plain_strng, uint8_t *key, uint8_t d, uint8_t *ciph_strng)
* 功    能：DES加密/解密函数：8字节的数据数组与8字节的密钥数组
* 入口参数：*plain_strng：待加密/解密的数据数组
						*key：用于加密/解密的密钥数组
						d：加密/解密的选择 0表示加密，1表示解密
...
void rotate_l(uint8_t* key) {
	...
	for (i = 0; i < 7; ++i) {
		key[i] = (key[i] << 1);
		if ((i < 6) && ((str_x[i + 1] & 128) == 128)) key[i] |= 1;
	}
	if (str_x[0] & 0x80) key[3] |= 0x10;
	else key[3] &= ~0x10;
	if (str_x[3] & 0x08) key[6] |= 0x01;
	else key[6] &= ~0x01;
}
/************************************************************************/
/*                                                                      */
