...
char AsciiTable8x16[95][16]={//粗体8x16点阵的ASCII码的点阵数据，从"JLX-GB2312"型号的字库IC中读出来的国标的。
0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,		//- -   ASCII码：0X20(即“空格”）
...
char AsciiTable5x8[95][5]={//全体ASCII 列表:5x8点阵
...
namespace FontRom{
	...
	u8 convert(u8 data){
		return (((data&0x80)>>7) | ((data&0x40)>>5) | ((data&0x20)>>3) | ((data&0x10)>>1) | ((data&0x08)<<1) | ((data&0x04)<<3) | ((data&0x02)<<5) | ((data&0x01)<<7));
	}
	void ReadWrite16x16(u32 fontaddr,u8 reverse,u8 page,u8 column){
	//从指定地址读出数据写到液晶屏指定（page,column)座标中
		u8 i,j,n,disp_data;
		*CS.O=0;
		spi2.ReadWriteByte(0x03);
		spi2.ReadWriteByte((fontaddr&0xff0000)>>16);//地址的高8位,共24位
		spi2.ReadWriteByte((fontaddr&0xff00)>>8);//地址的中8位,共24位
		spi2.ReadWriteByte(fontaddr&0xff);//地址的低8位,共24位
		for(j=0;j<2;j++){
			if(j==1) n=0;
			else if(j==0) n=1;
			//if((page==8)&&(n==0)) page =
			LCD12864::SetAddr(page+n,column);
			for(i=0; i<16; i++ ){
				if(reverse==1){
					disp_data=~spi2.ReadWriteByte(0x00);
				}else{
					disp_data=spi2.ReadWriteByte(0x00);
				}
				disp_data = convert(disp_data);
				LCD12864::WriteData(disp_data);//写数据到LCD,每写完1字节的数据后列地址自动加1
			}
		}
		*CS.O=1;
	}
		...
		*CS.O=0;
		...
		for(j=0;j<2;j++){
			...
			for(i=0; i<8; i++ ){
				...
				data = convert(disp_data);
				LCD12864::WriteData(data);	//写数据到LCD,每写完1字节的数据后列地址自动加1
			}
		}
		*CS.O=1;
	}
