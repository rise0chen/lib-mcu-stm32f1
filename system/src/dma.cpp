...
namespace dma{
	...
	void configTx(DMA_Channel_TypeDef* DMA_Chx, u32 cpar, u32 cmar, u16 cndtr){
		DMA_Chx->CMAR = cmar;//内存基址
		DMA_Chx->CPAR = cpar;//外设基址
		DMA_Chx->CNDTR= cndtr;//DMA缓存大小
		DMA_Chx->CCR = 0x91;//低优先 数宽8 内增外不 不循环 内到外 无中断
	}
	void configRx(DMA_Channel_TypeDef* DMA_Chx, u32 cpar, u32 cmar, u16 cndtr){
		DMA_Chx->CMAR = cmar;//内存基址
		DMA_Chx->CPAR = cpar;//外设基址
		DMA_Chx->CNDTR= cndtr;//DMA缓存大小
		DMA_Chx->CCR = 0x81;//低优先 数宽8 内增外不 不循环 外到内 无中断
	}
	void configADC(u32 cmar, u8 cndtr){
