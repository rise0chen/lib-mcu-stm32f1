...
Usage:
	rcc_init(9); //HSE晶振9倍频
	rcc_reset(2, APB2_GPIOA); //复位GPIOA
	rcc_cmd(2, APB2_GPIOA, ENABLE);//使能GPIOA时钟
...
uint32_t rcc_clkSys;
uint32_t rcc_clkAhb;
uint32_t rcc_clkApb1;
uint32_t rcc_clkApb2;
...
//配置向量表
#ifdef VECT_TAB_RAM
	nvic_setVectorTable(SRAM_BASE,0x0);
#else
...
void rcc_init(uint8_t PLL){
	...
	rcc_clkSys  = 72000000;
	rcc_clkAhb  = rcc_clkSys;
	rcc_clkApb1 = rcc_clkAhb>>1;
	rcc_clkApb2 = rcc_clkAhb;
}
